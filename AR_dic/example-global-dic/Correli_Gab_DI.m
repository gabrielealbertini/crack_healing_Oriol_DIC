...
Correlipath = 'D:\Imagerie\Correli\'; %fullfile(pwd,'..','..','..');
...
flag_mes_ini = 1; % mesh.tag of initial mesh
flag_mes_cur = 1; % mesh.tag of current mesh
...
for im = imini:imdec:imfin %numel(images)-1
    co = co+1;
    % load the images
    go = single(imread(images{im}));
    go = go(:,:,1);
    g = single(imread(images{im+imdec}));
    g = g(:,:,1);
    if flag_ini == 0
        if co==1
