...
% ======================================================
imagepath = '../example-images';
images = dir([imagepath, filesep, 'image_*.bmp']);
images = {images.name}.';
N = numel(images);

% number of steps (inc 267 doesn't have c and d)
Ninc = 15; % can be different from N
Nmin = 4;  % first deformed image to be correlated
% Pixel size (meter)
pix2m = 50e-3 / 1168; % PARAMETER TO ENTER
...
if false
    ...
    figure('Position',[150 50 1200 700])
    colormap(gray)
    imagesc(f)
    daspect([1 1 1]);
    colorbar
...
Young=3.0e3;           % Young's modulus (MPa)
...
plot(Mesh.coordinates(Contour,1),Mesh.coordinates(Contour,2),'b-','Linewidth',2);
...
D(1).convcrit = 1; %arrest criterion on crack tip positions (pixel)
D(1).maxit = 100;
D(1).trustregion = 10; %max increment of crack offset - to avoit overshoot
D(1).dampen = 0.6;
D(1).alpha = 0;
...
fig = figure('Position',[150 50 1200 800]);
colormap(gray)
hdl_img = imagesc(f);
daspect([1 1 1]);
axis image
...
hdl_cnt = plot(Mesh.coordinates(Contour,1),Mesh.coordinates(Contour,2),'b-','Linewidth',2);
hdl_tip = plot(crackpos(:,1),crackpos(:,2),'.r');
plot(crackpath.pos(:,1),crackpath.pos(:,2),'g-')
caxis([0 255])
drawnow
...
%     refim_name = sprintf('.*%03d%s.bmp',inc,'c');
%     defim_name = sprintf('.*%03d%s.bmp',inc,'a');
...
%     defim = find(not(cellfun(@isempty,regexp(images,defim_name))),1)
    ...
    % store some results
    results(inc,1) = D(1).a;
    results(inc,2) = D(1).da;
    results(inc,3) = D(1).K1;
    results(inc,4) = D(1).K2;
    results(inc,5) = D(1).TS;
    results(inc,6) = D(1).cor.r;
...
%     Contour = mesh_contour(D.Mesh);
    ...
    delete(hdl_cnt);
%     delete(hdl_tip);
    hdl_cnt = plot(D.Mesh.coordinates(Contour,1),D.Mesh.coordinates(Contour,2),'b-','Linewidth',2);
    hdl_tip = plot(crackpos(:,1),crackpos(:,2),'.r');
    caxis([0 255])
...
saveas(fig,'crack_path.png')
save('results.mat','results')
%%
fig=figure
plot(results(:,1),'bs-')
xlabel('picture#')
ylabel('Crack length, pixels??')
saveas(fig,'crack_length.png')
...
plot(results(:,4),'k+-')
xlabel('picture#')
ylabel('SIFs, MPa.m^{1/2}')
legend('K_I','K_{II}','Location','best')
saveas(fig,'KI_KII.png')

fig = figure
plot(results(:,5),'gh-')
xlabel('picture#')
ylabel('T-stress, MPa')
saveas(fig,'T-stress.png')

fig = figure
plot(results(:,6),'bs-')
xlabel('picture#')
ylabel('RMS residual, gray level')
saveas(fig,'RMS_residual.png')